...
/* 创建gdt描述符 */
struct gdt_desc make_gdt_desc(uint32_t *basc_addr, uint32_t limit,
                              uint8_t attr_low, uint8_t attr_high) {
  uint32_t desc_base = (uint32_t)basc_addr;
