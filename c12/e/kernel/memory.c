...
void mem_init() {
  ...
  /* 初始化mem_block_desc数组descs，为malloc 做准备 */
...
//     descs = (struct mem_bolck_desc *)k_block_descs;
//   } else { // 用户进程pcb中的pgdir会在为其分配页表时创建
//     PF = PF_USER;
//     pool_size = user_pool.pool_size;
//     mem_pool = &user_pool;
//     descs = (struct mem_bolck_desc *)cur_thread->u_block_desc;
//   }
...
//       }
//     }
//     /* 若mem_block_desc的free_list 中已经没有可用的mem_block ，
//      * 就创建新的arean提供mem_block */
...
void *sys_malloc(uint32_t size) {
  ...
  if (size > 1024) {
    ...
    for (desc_idx = 0; desc_idx < DESC_CNT; ++desc_idx) {
      if (size <=
          descs[desc_idx]
              .block_size) { // 从小往大依次寻找，找到第一个比所需大的即可退出
        break;
      }
    }
